...
// Create an empty array to hold lesson objects
var myWork = [];
// Use a for loop to create lessons
for (var i_1 = 1; i_1 <= 10; i_1++) {
    // Determine the lesson name
    var lessonName = "Lesson ".concat(i_1);
    // Set the status based on whether the lesson number is odd or even
    var isEven = i_1 % 2 === 0;
    var lessonStatus = isEven;
    // Create the lesson object
    var lesson = { name: lessonName, status: lessonStatus };
...
// 1. Set the maximum value (e.g., 100)
var maxValue = 100;
// 2. Generate a random number between 1 and maxValue
var randomNumber = Math.floor(Math.random() * maxValue) + 1;
console.log("Random number: ".concat(randomNumber)); // For development purposes
// 3. Initialize guess status
var correctGuess = false;
// 4. Predefined guesses (you can customize this array)
var predefinedGuesses = [42, 75, 60, 90, 30];
// 5. Iterate over guesses using a while loop
var i = 0;
while (!correctGuess && i < predefinedGuesses.length) {
    var userGuess = predefinedGuesses[i];
    // 6. Check user's guess
    if (userGuess === randomNumber) {
        console.log("Congratulations! You guessed correctly: ".concat(userGuess));
        correctGuess = true;
    }
    else if (userGuess < randomNumber) {
        console.log("Try a higher number than ".concat(userGuess, "."));
    }
    else {
        console.log("Try a lower number than ".concat(userGuess, "."));
    }
    i++;
}
if (!correctGuess) {
    console.log("Sorry, you didn't guess the correct number (".concat(randomNumber, ")."));
}
...
var counter = 0;
// 2. Define the step value
var step = 5; // You can customize this value
// 3. Create a do...while loop
do {
    console.log("Counter value: ".concat(counter));
    counter += step; // Increment the counter by the specified step
} while (counter < 100);
console.log("Reached or exceeded 100!");
...
// console
// Create a simple object with three properties
var myObject = {
    item1: "Value 1",
    item2: "Value 2",
    item3: "Value 3",
};
// Use a for...in loop to get property names and values
for (var propertyName in myObject) {
    var propertyValue = myObject[propertyName];
    console.log("".concat(propertyName, ": ").concat(propertyValue));
}
...
// 1. Create an empty array
var myArray = [];
// 2. Run a loop 10 times, adding incrementing values
...
for (var j = 0; j < myArray.length; j++) {
    console.log("Index ".concat(j, ": ").concat(myArray[j]));
}
// 5. Use the for...of loop to output the values directly
