...
console.log(`Random number: ${randomNumber}`); // For development purposes

// 3. Initialize guess status
let correctGuess = false;

// 4. Predefined guesses (you can customize this array)
const predefinedGuesses = [42, 75, 60, 90, 30];

// 5. Iterate over guesses using a while loop
let i = 0;
while (!correctGuess && i < predefinedGuesses.length) {
    const userGuess = predefinedGuesses[i];

    // 6. Check user's guess
    if (userGuess === randomNumber) {
        console.log(`Congratulations! You guessed correctly: ${userGuess}`);
        correctGuess = true;
    } else if (userGuess < randomNumber) {
        console.log(`Try a higher number than ${userGuess}.`);
    } else {
        console.log(`Try a lower number than ${userGuess}.`);
    }
...
if (!correctGuess) {
    console.log(`Sorry, you didn't guess the correct number (${randomNumber}).`);
} 
...
const step = 5; // You can customize this value
